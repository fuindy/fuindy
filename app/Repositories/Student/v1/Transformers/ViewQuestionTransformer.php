...
class ViewQuestionTransformer extends TransformerAbstract
{
    /**
    ...
    public function transform(StudentQuestionTest $questionTest)
    {
        return [
            ...
            'question.js' => [
                ...
                'detail' => !is_null($questionTest->question) ? $this->handleQuestionDetail($questionTest->question->id) : array(),
            ],
        ];
    }

    private function handleQuestionDetail($questionId)
    {
        $question = QuestionDetail::where('question_id', $questionId)->get();
